      ...
      (let open Command.Let_syntax in
       ...
       fun () ->
         ...
         List.iter files ~f:(fun filename ->
           ...
           Eio_writer.printf stdout "================================: %s\n" filename;
           match pretty_print ~env ~path ~read_contents_from_stdin:false with
           | Error e -> Eio_writer.print_endline stdout (Error.to_string_hum e)
           | Ok { pretty_printed_contents; result } ->
             Eio_writer.print_string stdout pretty_printed_contents;
             (match result with
              | Ok () -> ()
              | Error e ->
                Eio_writer.printf stdout "======: errors\n%s\n" (Error.to_string_hum e))))
  ;;
       ...
       fun () ->
         ...
         List.iter files ~f:(fun file ->
           List.iter (generate_rules ~file) ~f:(fun sexp ->
             Eio_writer.print_s stdout sexp)))
  ;;
       ...
       fun () ->
         ...
         match
           ...
           Eio_writer.with_flow (Eio.Stdenv.stdout env) (fun stdout ->
             Eio_writer.print_string stdout pretty_printed_contents;
             if add_extra_blank_line then Eio_writer.print_newline stdout);
           result
         with
         | Ok () -> ()
         | Error e ->
           Eio_writer.with_flow (Eio.Stdenv.stderr env) (fun stderr ->
             Eio_writer.print_endline stderr (Error.to_string_hum e));
           Stdlib.exit 1)
  ;;
       ...
       fun () ->
         ...
         match
           ...
           Ref.set_temporarily Loc.include_sexp_of_positions with_positions ~f:(fun () ->
             Eio_writer.with_flow (Eio.Stdenv.stdout env) (fun stdout ->
               Eio_writer.print_s stdout [%sexp (program : T.t)]));
           return ()
         with
         | Ok () -> ()
         | Error e ->
